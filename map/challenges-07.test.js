...
const forLoopTwoToThe = (input) => {
  ...
  for(let i = 0; i < input.length; i++){
    results.push(Math.pow(2,input[i]))
  }
  //   return results;
  return results;
};
...
const forEachTwoToThe = (input) => {
  // Solution code here...
  let results = [];
  input.forEach(function(idx){
    results.push(Math.pow(2,idx))
  })
  return results;
};
...
const extractAbilities = (abilities) => {
  // Solution code here...
 return abilities.map(idx => idx.ability.name)
  }
