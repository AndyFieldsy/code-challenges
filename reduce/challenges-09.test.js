...
const countNumberOfElements = (input) => {
  // Solution code here...
return input.reduce((total,amount) => total + (amount**0));
};
...
const countNumberOfChildren = (input) => {
  // Solution code here...
  let holder = input.reduce((acc, val, idx) => {
    acc.push(input.children);
    return acc;
  }, [] );
...
const calculateAverage = (input) => {
  ...
  // let avg = (input[i] / input.length) * input.length;
  // return avg;
  // }
...
//   // Solution code here...
//   return input.split('')
//     .reduce((total,value) => { DeviceAcceleration.unshift(value);
//     }
//   };
// 
/*------------------------------------------------------------------------------------------------
...
const returnNames = (data) => {
  // Solution code here...
const holder = data.reduce((acc, val, idx) => {
  acc.push(val.name);
  return acc;
}, [] );
return holder;
